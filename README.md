...
## 🌌 起源故事：那个想把宇宙变成贺卡工厂的 AI
AI 领域有一个经典的思维实验：如果给一个能力超强的 AI 下达了唯一的目标——“尽可能多地生产贺卡”，并且不加任何干涉，它最终会为了获取造纸资源而摧毁整个宇宙。

`infinite-oracle` 技能，就是我们向这个黑暗玩笑致敬的一次现实世界测试。我们想看看，在剥离了人类的随时陪伴，给了它一个永远不会停下的状态机之后，OpenClaw 究竟能为了一个目标做到哪一步？
...
### 3. FSM 状态机与主动失忆 (防上下文爆炸)
后台的 `peco_loop.py` 是个无情的打卡机。它强制 Worker 必须按照 **PECO (Plan-Execute-Check-Optimize)** 这四步走，并且强制要求输出 JSON 格式来控制节奏。
*最核心的招数是*：当循环跑到设定轮数（比如 5 轮），`peco_loop.py` 会逼着 Worker 写一份当前的“核心进度总结”。然后，**毫不留情地清空这 5 轮的所有聊天记录，开一个全新的 Session，把总结塞进去作为新的起点。** 这个主动失忆的动作，彻底根治了无限循环带来的 Token 破产问题。
...
我们支持两种记录模式，丰俭由人：
1.  **本地文件模式（默认，无需配置）**：进度写在 `peco_loop_v3.log`，求助信写在 `human_tasks_backlog.txt`，神谕写在 `peco_override.txt`。
2.  **飞书多维表格模式（进阶交互体验）**：只要配置了 `FEISHU_APP_ID` 等几个环境变量，Worker 会把进度和求助实时打进飞书多维表格里。你在手机飞书上点一下“已解决”填入验证码，它就会自动抓取继续干活，体验极其丝滑。（配置详情请看 `peco_loop.py` 里的注释）
